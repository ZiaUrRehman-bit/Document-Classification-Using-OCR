...
parentDir = "C:\\Users\\hp\\Google Drive\\Fiverr Work\\2022\\22. Document Classification Using OCR"
embedded = "Embedded"
android = "Android"

path1 = os.path.join(parentDir, embedded)
path2 = os.path.join(parentDir, android)
try:
    os.mkdir(path1)
    os.mkdir(path2)
except:
    ...
    if x!=0:
        ...
        if len(b) == 12:
            ...
            if b[11] == "Embedded":
                ...
                # change Directory
                os.chdir(r"C:\\Users\\hp\\Google Drive\\Fiverr Work\\2022\\22. Document Classification Using OCR\\Embedded")
                
                # now save the image
                cv.imwrite(f"Embedded.png", img)
                # now we have to label thses characters 
            ...
            if b[11] == "Android":
                ...
                # change Directory
                os.chdir(r"C:\\Users\\hp\\Google Drive\\Fiverr Work\\2022\\22. Document Classification Using OCR\\Android")
                
                # now save the image
                cv.imwrite(f"Android.png", img)
                # now we have to label thses characters 
                cv.putText(newImg, b[11], (x, y-5), cv.FONT_HERSHEY_COMPLEX, 1, (0,0,255), 2)
